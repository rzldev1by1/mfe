    ...
    return (
      <Container className="px-5 py-5 tab-content">
        <Col className="pl-4 pb-4 mb-1">
          ...
          ></i>
          <div className="h1 bold text-gray-title font-2xl mt-4">
            {status !== 'Successfully added' && status !== 200 ? 'Sorry' : ''}
            {status === 'Successfully added' ? 'Success' : ''}
            {status === 200 ? 'Thank You' : ''}
          </div>
          <div className="h4 text-gray w-75 msg-tab text-gray-md font-2xl">
            {statusMessage}
            <span className="h4 text-gray w-50 font-2xl">
              {status === 'Successfully added' || status === 200 ? null : 'Please try again.'}
            </span>
          </div>
          <Row className="flex justify-content-between align-items-center">
            ...
            <div className="text-right vertical-middle h-fit-content e-text-color">{`${
              status === 'Successfully added' || status === 200 ? 'Code 200' : 'Error Code 400'
            }`}</div>
          </Row>
        </Col>
      </Container>

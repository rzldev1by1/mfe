...
class AutoComplete extends Component{
                        ...
                        if(idx >= (datacount.length - 1)){
                            return(
                                    <li key={idx + data} className="select_autocomplete_option">
                                        <input className="select_autocomplete_input" type="radio" name={"select" + placeHolder + this.state.no} value={optionListValue[idx]} checked={this.state.data === optionListValue[idx] ? true : false} onChange={(e) => {getValue(e.target.value, data); this.setState({ data: e.target.value }); if(this.props.isOpen){this.props.isOpen(false)}}} id={"select-" + data + this.state.no} defaultChecked={optionSelected === data || optionSelected === optionListValue[idx] ? true : false}/>
    ...
    render(){
        ...
        return(
            <React.Fragment>
                <ul className={"select_autocomplete " + this.props.className  + (this.state.data === "closeAutocomplete" ? "autocomplete_closed" + this.props.className : "" ) + ((this.state.data === "openAutocomplete") && (this.props.usedFor === "SalesOrderCreate") ? " autoCompleteForOrderLine" : "" )} style={ style } tabIndex={this.props.tabIndex} > 
                    <input type="text" ref="searchInput" id="search" style={ this.props.uppercase ? { textTransform: "uppercase", height: "50px !important" } : { height: "50px !important" }} name="search" className={"form-control search-input-placeholder " + (this.state.data === "openAutocomplete" ? "search-input search-input-focus" : "")} onChange={(e) => {this.onChange(e);}} autoComplete="off" onFocus={(e) => {if(this.props.optionList === undefined) {getValue("")}; this.setState({ data: "openAutocomplete" }); if(this.props.isOpen){this.props.isOpen(true)}}} placeHolder={placeHolder} value={(this.state.data != "openAutocomplete") && (this.state.data != "closeAutocomplete") ? this.state.data : (this.props.optionSelected ? this.props.optionSelected : null)} maxLength={this.props.maxLength}/>
                    ...
                    <li className={"select_autocomplete_items" + (this.state.data != "openAutocomplete" ? " select_autocomplete_items_closed" : "")}>
                        ...
                        <label className="select_autocomplete_closeLabel" htmlFor={"select-close" + placeHolder + this.state.no} onClick={() => {if(this.props.isOpen){this.props.isOpen(false)}}}></label>

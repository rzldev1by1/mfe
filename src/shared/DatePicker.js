...
function Navbar({
  ...
  return (
    <div
      className={className}
      style={{ marginTop: '14px', marginLeft: '19px', marginRight: '19px', position: 'relative' }}
    >
...
class DatePicker extends React.Component {
      ...
      if (moment(prevProps.firstDate).format('DD/MM/YYYY') !== moment(this.props.firstDate).format('DD/MM/YYYY')) {
        if (!this.props.firstValue) {
          ...
          this.setState({
            ...
            defaultValue: moment(firstDate).format('DD/MM/YYYY'),
          });
  ...
  handleDayClick = (day, modifiers = {}) => {
    ...
    this.setState({
      selectedDay: day,
      defaultValue: moment(day).format('DD/MM/YYYY'),
    });
    ...
    if (e.target.value.length >= 10) {
      ...
      if (month <= 12 && date <= 31) {
        this.setState({
          ...
          month: new Date(year + '-' + month + '-' + date),
        });
    ...
    return (
      <React.Fragment>
        <ul className={className} style={this.props.style} tabIndex={this.props.tabIndex ? this.props.tabIndex : null}>
          ...
          <input
            className={
              'select_date_expand' + (this.props.arrowStyle ? ' select_arrow_expand' : ' select_calendar_expand')
            }
            ref="opener"
            type="checkbox"
          ...
          >
            {({ width, height }) => (
              <div
                ...
                className={
                  'select_date_options ' +
                  (this.props.field === 'smallField ' ? ' smallField ' : '') +
              ...
              >
                <div className="dateInfo">
                  {this.state.selectedDay
                    ? moment(this.state.selectedDay).format(this.props.shortFormat ? 'DD MMM YYYY' : 'DD MMMM YYYY')
                ...
                <DayPicker
                  ...
                  fromMonth={
                    this.props.fromMonth ? new Date(this.props.fromMonth) : new Date(new Date().getFullYear() - 5, 0)
                  }
                  toMonth={
                    this.props.toMonth
                      ? new Date(new Date(this.props.toMonth).getFullYear(), 11)
                      : new Date(new Date(this.state.month).getFullYear() + 10, 11)
                  }
                  disabledDays={

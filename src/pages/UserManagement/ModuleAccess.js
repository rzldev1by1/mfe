  ...
  <div>
        <div className="flex-column mr-2">
          <div className="d-flex">
            <label className="col-6 title-label px-0" htmlFor="moduleAccess">Module Access
            </label>
           ...
           (moduleAccess && moduleAccess.length)? moduleAccess.map((item,index) => {
              ...
              return (<div className="flex-column mb-3 mr-2" key={index}>
                            <div className="d-flex" key={index}>
                                <label className="col-6 section-value-text px-0" key={item.menuid}>{(item.menuname.toLowerCase() === 'create sales order')?'Sales Orders':item.menuname}</label>

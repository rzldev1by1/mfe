...
class NewUser extends React.PureComponent {
  ...
  isFormValid = () => {
    const { user, moduleAccess, clients, sites, isAdmin } = this.props
    const { validation } = this.state
    let formValid = false

    if (sites && clients && moduleAccess) {
      const activeClient = !isAdmin ? clients.filter((c) => c.status !== false) : ["Admin"];
      const activeSite = !isAdmin ? sites.filter((s) => s.status !== false) : ["Admin"];
          ...
          <Container className="px-5 pt-4 pb-5">
              ...
              <Row className="mt-5">
                  ...
                  <Col lg={2} className="text-right">
                      <button className={`btn font-lg ${!isFormValid ? 'btn-grey' : 'btn-primary'}`} disabled={!isFormValid} onClick={(e) => { this.onNext(); }}>{'NEXT'}</button>
                  </Col>
              </Row>
          </Container>
      )

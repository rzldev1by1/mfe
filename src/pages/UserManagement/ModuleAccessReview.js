            ...
            (moduleAccess && moduleAccess.length)?moduleAccess.map((item,index) => {
              ...
              return (<div className="flex-column mb-1 mr-2" key={index}>
                            <div className="row height-40" key={index}>
                                ...
                                <div className="col-6">
                                  <button type="button" className={"px-1 font-lg no-outline "+((item.status)?"enable-active":"enable-notactive")}>{`${item.status?enable.toUpperCase():disable.toUpperCase()}`}</button>
                                </div>
                            </div>
                      </div>)
